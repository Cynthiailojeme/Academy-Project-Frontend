      ...
      <form @submit.prevent="add" v-if="isApplicationStillOpen">
        <div class="form-group col-md-8">
          ...
          <div v-else>
            ...
            <input
              class="first_box2 form_control form-group col-md-7"
              id="dotted"
              v-model="formData.image"
              name="image"
            />
            <button class="btn3" @click="removeImage">Remove file</button>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.myWidget = cloudinary.createUploadWidget(
      {
        cloudName: "dygctamiz",
        uploadPreset: "ht58on2s"
      },
      (error, result) => {
        if (!error && result && result.event === "success") {
          console.log("Done! Here is the image info: ", result.info);
          console.log("Image Url " + result.info.url);
          this.formData.image = result.info.url;
        } else {
          console.log(error);
        }
      }
