    ...
    <div class="top">
      <div class="left">
        ...
        <span
          ...
          v-show="disallowassessment===false">Thank you!</span>
        <span class="warning" v-if="disallowassessment===true">
          Sorry, this assessment can only be taken once. Please check your dashboard for updates. Thanks
        </span>
      </div>

      <div class="right" v-show="disallowassessment===false" v-if="showquestions===true">
        Timer:
    ...
    <div v-if="takenassessment==false" >
      <div v-if="showquestions===true">
        <div
          ...
          >
          <p class="numbering" :id="quizIndex">Question {{ quizIndex+1 }}</p>
          <div class="quizzes">
              <p class="title" :id="quiz._id" ref="id">{{ quiz.question }}</p>
              <p class=".image-bind"><img :src="quiz.img" alt="image" v-if="quiz.img"></p>
              <div v-for="(option, index) in quiz.options" :key="index" class="rad" style="margin-left: 10px;">
                  <input type="radio" name="muhRadio" v-bind:value="option" :id="index"/><label for="muhRadio" class="option">{{ option }}</label>
              </div>
              <br/>
          </div>
        </div>
